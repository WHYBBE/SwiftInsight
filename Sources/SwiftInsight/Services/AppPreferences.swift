...
enum L10n {
    ...
    private static let zh: [String: String] = [
        ...
        "settings.helper.body": "活动监视器通过 sysmond + 私有 entitlement 读取系统保护进程。第三方 App 默认不行。自用可安装 setuid root Helper，用同一套 libproc 在 root 下采样并回传给主程序。",
        "settings.helper.install": "安装（需管理员密码，仅本机自用）：",
        "settings.helper.uninstall": "卸载：sudo rm -f /usr/local/libexec/SwiftInsightHelper",
        "settings.helper.recheck": "重新检测 Helper",
        "settings.helper.ok": "已安装 · root 生效",
        "settings.helper.not_root": "已找到但非 root（请重新 install）",
    ...
    private static let en: [String: String] = [
        ...
        "settings.helper.body": "Activity Monitor uses sysmond + private entitlements for protected processes. Third-party apps cannot. For personal use, install a setuid root Helper that samples via libproc and returns data to the app.",
        ...
        "settings.helper.recheck": "Recheck Helper",
        "settings.helper.ok": "Installed · root active",
        "settings.helper.not_root": "Found but not root (re-run install)",
